...
- (void)viewDidLoad
{
    ...
    if ([UIScreen mainScreen].bounds.size.height>500) {
        [_ctrView setResultFrame: CGRectMake(10, 120+80, 300, 80)];
        wordFrame = CGRectMake(10, 20, 300, 100 + 80);
    }
    [_ctrView setResultTextAlignment:NSTextAlignmentLeft];
    [_ctrView setText:@"识别结果："];
...
- (BOOL)clickedStartBtn{
    [_ctrView setText:@""];
    return [_wxssui showAndStartOnceWithGrammarString:_grammarTextView.text andType:(int)[self grammarType]];
}
...
- (void)voiceInputResultArray:(NSArray *)array{
    if (array && array.count>0) {
        ...
        [_ctrView setText:[NSString stringWithFormat:@"识别结果：%@", result.text]];
    }else{
        [_ctrView setText:@""];
    }
}
