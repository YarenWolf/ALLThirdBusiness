...
- (void)setRecordingState:(DDRecordingState)recordingState
{
    switch (recordingState)
    {
        ...
        case DDShowRecordTimeTooShort:
            ...
            break;
        case DDShowTheRestTimeRemind:
            [self ShowTheRestTimeRemind];
            break;
    }
}
...
- (void)setVolume:(float)volume
{
    ...
    DLog(@"___________%f",height);
    CGRect frame = volumnImageView.frame;
    frame.size.height = height;
    frame.origin.y = 126-height;//height - frame.size.height;
...
- (void)setupShowTheRestTimeRemind
{
    [self.subviews enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        if ([(UIView*)obj tag] != 100)
        {
    ...
    UILabel* prompt = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, DDVIEW_RECORDING_WIDTH, DDVIEW_RECORDING_HEIGHT)];
    [prompt setBackgroundColor:[UIColor clearColor]];
    [prompt setTextColor:[UIColor whiteColor]];
    [prompt.layer setCornerRadius:2];
    [prompt setTextAlignment:NSTextAlignmentCenter];
    [prompt setFont:[UIFont systemFontOfSize:80]];
    prompt.tag=DDVOLUMN_LABLE_TAG;
    [prompt setText:@""];
...
-(void)ShowTheRestTimeRemind{
    //倒计时
    if (self.recordingState != DDShowTheRestTimeRemind)
    {
        [self setupShowTheRestTimeRemind];
    }
    _recordingState = DDShowTheRestTimeRemind;
    
    
}
- (float)heightForVolumn:(float)vomlun
{
    vomlun= vomlun>1.0?1.0:vomlun;
    vomlun= vomlun<0.0?0.0:vomlun;
    //0-1.0 volumn
    float height = 43.0 / 1.0 * vomlun;
    return height;
}
-(void)setTheRestTimeRemindNum:(int)num{
    //
    
    if (_recordingState != DDShowTheRestTimeRemind)
    {
        return;
    }
    UILabel *lblnum=(UILabel *)[self viewWithTag:DDVOLUMN_LABLE_TAG];
