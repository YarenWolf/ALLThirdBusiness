...
@interface EGORefreshChatTableHeaderView (Private)
- (void)setState:(EGOPullRefreshState)aState;
@end

@implementation EGORefreshChatTableHeaderView
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
		...
		label = [[UILabel alloc] initWithFrame:CGRectMake(0.0f, RefreshViewHight - 10.0f, self.frame.size.width, 20.0f)];
		...
		view.frame = CGRectMake(65.0f, RefreshViewHight - 10.0f, 20.0f, 20.0f);
...
- (void)setState:(EGOPullRefreshState)aState{
	...
	switch (aState) {
		case EGOOPullRefreshPulling:
			...
			_statusLabel.text = ssssss(@"加载历史记录...", @"加载历史记录...");
			[CATransaction begin];
		...
		case EGOOPullRefreshNormal:
			...
			_statusLabel.text = ssssss(@"下拉即可加载历史记录...", @"上拉即可加载历史记录...");
		...
		case EGOOPullRefreshLoading:
			...
			_statusLabel.text = ssssss(@"加载中...", @"加载中...");
...
- (void)egoRefreshScrollViewDidScroll:(UIScrollView *)scrollView {
	if (_state == EGOOPullRefreshLoading) {
		...
		if (_state == EGOOPullRefreshPulling && scrollView.contentOffset.y > -RefreshViewHight && scrollView.contentOffset.y < 0.0f && !_loading) {
			[self setState:EGOOPullRefreshNormal];
		} else if (_state == EGOOPullRefreshNormal && scrollView.contentOffset.y < -RefreshViewHight-20 && !_loading) {
