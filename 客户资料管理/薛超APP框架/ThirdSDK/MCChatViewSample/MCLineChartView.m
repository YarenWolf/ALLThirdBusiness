...
- (void)reloadChartDataSource {
    ...
    _cachedMinHeight = kChartViewUndefinedCachedHeight;
    _lineCount = 1;
    _lineCount = self.dataArray.count;
    ...
    for (NSUInteger i = 0; i < _lineCount; i ++) {
        NSUInteger lineCount = self.lineCountBlock(i);
        ...
        for (NSInteger j = 0; j < lineCount; j ++) {
            id value = self.lineValueBlock(i,j);
...
- (void)reloadLineWithAnimate:(BOOL)animate {
    ...
    for (NSInteger lineNumber = 0; lineNumber < _lineCount; lineNumber ++) {
        ...
        if (self.colorBlock) {
            CGColorRef color = self.colorBlock(lineNumber).CGColor;
        ...
        if (self.lineWidthBlock) {
            lineLayer.lineWidth = self.lineWidthBlock(lineNumber);
            pointLayer.lineWidth = self.lineWidthBlock(lineNumber);
        } else {
        ...
        for (NSInteger index = 0; index < array.count; index ++) {
            ...
            if (self.pointViewBlock) {
                UIView *view = self.pointViewBlock(lineNumber,index);
                if (view) {
            ...
            */
            ...
            if (self.hintViewBlock) {
                UIView *hintView = self.hintViewBlock(lineNumber,index);
                if (hintView) {
            ...
            } else if (self.informationBlock) {
                NSString *information = self.informationBlock(lineNumber,index);
                if (information) {
            ...
            if (lineNumber == 0 && self.titleBlock(lineNumber)) {
                ...
                textLabel.numberOfLines = 0;
                textLabel.text = self.titleBlock(index);
...
-(void)fillDataWithParameters:(NSDictionary *)parameters dataArray:(NSMutableArray *)dataArray lineCountAtlineNumber:(lineCountBlock)lineCountBlock valueAtLineNumber:(lineValueBlock)lineValueBlock lineWidthWithLineNumber:(lineWidthBlock)lineWidthBlock titleAtLineNumber:(titleBlock)titleBlock lineColorWithLineNumber:(colorBlock)colorBlock hintViewOfDotInLineNumber:(hintViewBlock)hintViewBlock pointViewOfDotInLineNumber:(pointViewBlock)pointViewBlock informationOfDotInLineNumber:(informationBlock)informationBlock{
    [self setValuesForKeysWithDictionary:parameters];
    self.dataArray = dataArray;
    self.lineCountBlock = lineCountBlock;
    self.lineValueBlock = lineValueBlock;
    self.lineWidthBlock = lineWidthBlock;
    self.titleBlock = titleBlock;
    self.colorBlock = colorBlock;
    self.hintViewBlock = hintViewBlock;
    self.pointViewBlock = pointViewBlock;
